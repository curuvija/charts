...
A Helm chart to export query results from Oracle DB
...
| image | object | `{"pullPolicy":"IfNotPresent","repository":"ghcr.io/iamseth/oracledb_exporter","tag":"0.6.0"}` | Image to use for deployment |
...
| image.tag | string | `"0.6.0"` | Overrides the image tag whose default is the chart appVersion. |
...
| prometheus | object | `{"monitor":{"additionalLabels":{},"enabled":false,"interval":"15s","namespace":[],"path":"/metrics"}}` | configure Prometheus Service monitor to expose metrics |
| prometheus.monitor.additionalLabels | object | `{}` | add additonal labels to service monitoring |
| prometheus.monitor.enabled | bool | `false` | enable or disable creation of service monitor |
